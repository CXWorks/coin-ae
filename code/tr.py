...
model_name = "unsloth/Meta-Llama-3.1-8B-Instruct";load_in_4bit = False
...
def compute_metrics(eval_pred):
    ...
    predictions, true_labels = get_predictions(last_token_logits, labels[:, -1])
...
if __name__ == '__main__':
    ...
    #
    #     def set_weights(self, class_weight):
    #         self.class_weights = class_weight
    #
    #     def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
    #         # Handle loss kwargs if model accepts them
    ...
    #         #print('label', labels.shape, outputs.shape, logits.shape)
    #         # Get the indices of non-ignored labels (-100)
    #         valid_label_indices = (labels != -100).nonzero(as_tuple=True)
    #
    #         # Debug checkpoint
    #         # print('Logits:', logits)
    #         # print('Labels:', labels)
    #
    #         # Extract logits for the last valid token in each sequence
    #         last_token_indices = torch.zeros(logits.size(0), dtype=torch.long, device=torch.device("cuda:0"))
    #         for batch_idx in range(logits.size(0)):
    #             batch_valid_indices = valid_label_indices[1][valid_label_indices[0] == batch_idx]
    #             if len(batch_valid_indices) > 0:
    #                 last_token_indices[batch_idx] = batch_valid_indices[-1]
    #
    #         last_token_logits = logits[torch.arange(logits.size(0)), last_token_indices]
    #
    #         # Extract corresponding labels
    #         valid_labels = labels[torch.arange(labels.size(0)), last_token_indices]
    #
    #         # Ensure class weights are on the correct device
    #         class_weights = torch.tensor(self.class_weights, device=torch.device("cuda:0"))
    #
