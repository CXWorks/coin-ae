...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train LLM with data parallelism')
    ...
    if load_in_4bit:
        # For 4-bit quantization, we need to use device_map="auto"
        ...
        # For regular training, load on first GPU and then use DataParallel
        base_model = AutoModelForCausalLM.from_pretrained(
            pretrained_model_name_or_path=model_name,
            load_in_4bit=False,
        ).to(device)
    ...
    # Apply DataParallel if not using 4-bit quantization and multiple GPUs are available
    if not load_in_4bit and num_gpus > 1:
        print(f"Using DataParallel across {num_gpus} GPUs")
        model = nn.DataParallel(model)
    else:
        print("Not using DataParallel - either quantization is active or only one GPU is available")
    ...
    balanced_train_dataset = concatenate_datasets([label_0_random_sample, label_1_data]).shuffle(seed=args.seed)
    raw_datasets['train'] = balanced_train_dataset

    # Resize validation dataset to prevent OOM
    raw_datasets["validation"] = raw_datasets["validation"].shuffle(seed=args.seed).select(range(20000))
    ...
    # Adjust batch size based on number of GPUs when not using quantization
    effective_batch_size = 1
    grad_accum_steps = 8
    
    if not load_in_4bit and num_gpus > 1:
        # With DataParallel, we can increase the per_device_batch_size or decrease gradient accumulation
        # Option 1: Keep same total batch size by reducing gradient accumulation
        #grad_accum_steps = max(1, grad_accum_steps // num_gpus)
        #print(f"Adjusted gradient accumulation steps to {grad_accum_steps}")
        
        # Option 2 (alternative): Increase per_device_batch_size
        effective_batch_size = min(4, num_gpus)  # Scale batch size with number of GPUs
        print(f"Adjusted per-device batch size to {effective_batch_size}")

    # Define training arguments
    training_args = TrainingArguments(
        output_dir="outputs_8b_classifier",
        per_device_train_batch_size=effective_batch_size,  # Adjusted based on GPUs
        ...
        # Set dataloader workers based on GPUs
        dataloader_num_workers=max(4, 2 * num_gpus),
    )
