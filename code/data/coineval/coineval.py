...
def generate_prompt(text, st, ed, wst, wed):
    ...
    if wst == st and wed == ed:
        return ''.join(text), st, ed, is_test
    else:
        return ''.join(text[wst - 1: wed]), st, ed, is_test
...
class Coineval(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples_caller_callee(self, filepath, all, split, call_depth=3, max_insts_per_func=500):
        ...
        with open(f'/mnt/md0/xiang/coin2/coin_{split}.pkl', 'rb') as fp:
            data = pickle.load(fp)
            ...
            for f, text, ls, window in data:
                for idx, (st, ed) in enumerate(ls):
                    ...
                    prompt, _, _ , is_test = generate_prompt(text, st, ed, window[idx][0], window[idx][1])
                    if not is_test:
                        id += 1
                        yield id, {"file_location": f, "function_text": prompt}
        #         else:
        #             for f, text, ls, window in vv:
        #                 for idx, (st, ed) in enumerate(ls):
        #                     id += 1
        #                     prompt, _, _ = generate_prompt_unsafe(text, st, ed, window[idx][0], window[idx][1])
        #                     yield id, {"file_location": f, "function_text": prompt, 'label': 1}
        # with open(f'/mnt/ssd1/xiang/coin_data/coin_{split}.pkl', 'rb') as fp:
        #     data = pickle.load(fp)
        #     for f, text, ls in data:
        #         for st, ed in ls:
        #             id+=1
        #             prompt, _, _ = generate_prompt(text, st, ed)
        #             yield id, {"function_text": prompt}
