...
def load_model(checkpoint_path="outputs_sec/checkpoint-9000",
...
if __name__ == '__main__':
    ...
    batch_id = int(os.getenv('INFER_BATCH'))
    ...
    def formatting_test_func(dataset_):
        if isinstance(dataset_['function_text'], str):
            return {'function_text':prompt.format(dataset_['function_text'], '')}
        texts = []
    ...
    test_dataset = raw_datasets['test'].select(range(batch_id*10000, min(len(raw_datasets['test']), (batch_id+1)*10000))).map(
    ...
    for tokenized_input, test_str, file_loc in tokenized_inputs:
        length = tokenized_input['input_ids'].shape[1]
        grouped_inputs[length].append((tokenized_input, test_str, file_loc))
    ...
    for length, group in tqdm(grouped_inputs.items()):
        for i in range(0, len(group), batch_size):
            ...
            batch_locs = [item[2] for item in batch]
            ...
            predictions = (probabilities[:, 1] > 0.8).long()
    ...
    for i in range(len(all_outputs)):
        data_tuple.append((all_probabilities[i], all_strings[i], all_locs[i]))
    data_tuple.sort(key=lambda x: x[0], reverse=True)
    with open(f'data_{cuda}_{batch_id}.pkl', 'wb') as fp:
        pickle.dump(data_tuple[:50], fp)
    ...
    print(f"{cuda} {batch_id} Total: {total} 50 / {pred_t} {pred_f}")
