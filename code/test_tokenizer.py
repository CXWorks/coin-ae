...
def init_datasets(model_args, data_args):
    ...
    #     for i in range(len(raw_datasets[k])):
    #         wl.append((raw_datasets[k][i]['context'] + raw_datasets[k][i]['function_text']).strip())
    # print(len(wl))
    # tokenizer.train_from_iterator(wl,
    #                 vocab_size=32100,
    #                 special_tokens=old_tokenizer.all_special_tokens_extended,
    # )
    #
    # new_tokenizer = RobertaTokenizerFast(
    #     tokenizer_object=tokenizer,
    #     additional_special_tokens=old_tokenizer.all_special_tokens_extended[5:],
    #     model_max_length=514,
    #     vocab_size=old_tokenizer.vocab_size,
    # )
    # print(new_tokenizer)
    # print(old_tokenizer)
    new_tokenizer = AutoTokenizer.from_pretrained('rust_tokenizer')
    print(new_tokenizer)
    test="""
        #[inline]
    ...
    """
    ans = new_tokenizer.encode(test)
    for id in ans:
        print(id, new_tokenizer.decode(id))
    # tokenize datasets
...
def main():
    ...
    # Init datasets
    init_datasets(model_args, data_args)
