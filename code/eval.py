...
checkpoint = "/mnt/md0/xiang/RustUnsafeDetector/snapshots/codebert1/"
...
def compute_metrics(p):
    ...
    preds1 = np.argmax(predictions, axis=-1)
...
def train_and_eval(tokenized_datasets, model, tokenizer, model_args, data_args, training_args):
    ...
    print("Test set alternatives:", len(tokenized_datasets))
    ...
    predictions = trainer.predict(tokenized_datasets).predictions
    preds1 = np.argmax(predictions, axis=-1)
    ct = 0
    name = 'original_unsafe_candidate.log'
    if not training_args.do_eval:
        name = 'f_'+name
    else:
        name = 's_'+name
    ...
    with open(name, 'w') as sys.stdout:
        for i in range(len(preds1)):
            if preds1[i] == 1:
                print('='*20)
                ct += 1
                print(tokenized_datasets[i]['locator'])
                print('-'*20)
                print(tokenized_datasets[i]['function_text'])
        print(ct)
    return model
