...
def init_model():
    model = RobertaForSequenceClassification.from_pretrained('/mnt/md0/xiang/RustUnsafeDetector/snapshots/test_use/', num_labels=2)
    model.to(device)
    return model
...
def compute_metrics(pred):
    ...
    probabilities = pred.predictions.argmax(-1)
    precision, recall, thresholds = precision_recall_curve(labels, probabilities)
    with open('thres.pkl', 'wb') as fp:
        pickle.dump((precision, recall, thresholds), fp)
    best_f1 = -1
    best_pre = 0
    best_recall = 0
    best_threshold = 0
    for i in range(len(precision)):
        ...
        f1 = 2 * (final_precision * final_recall) / (final_precision + final_recall + 1e-10)  # Adding tiny value to avoid division by zero
        if f1 > best_f1:
            best_f1 = f1
            best_recall = final_recall
            best_pre = final_precision
            best_threshold = thresholds[i]
    print(best_f1, best_recall, best_pre, best_threshold)
    preds = pred.predictions.argmax(-1)

    precision, recall, f1, threshold = precision_recall_fscore_support(labels, preds, average='binary', pos_label=1)
